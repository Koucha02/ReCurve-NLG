...
c = las.data[4000:6000, 1]
z = c.astype('float32')
x = torch.from_numpy(z)
# d2l.plot(time, [x], 'time', 'x', xlim=[1, 2000], figsize=(6, 3))
# plt.show()
...
net = get_net()
train(net, train_iter, loss, 10, 0.001)

onestep_preds = net(features)
d2l.plot(
    [time, time[tau:]],
    [x.detach().numpy(), onestep_preds.detach().numpy()], 'time', 'x',
    legend=['data', 'l-step preds'], xlim=[1, 2000], figsize=(6, 3))
...
d2l.plot([time, time[tau:], time[n_train+tau:]], [
    ...
    multistep_preds[n_train+tau:].detach().numpy()], 'time', 'x',
         legend=['data', 'l-step preds', 'multistep preds'], xlim=[1, 2000], figsize=(6, 3))
plt.show()
